...
/** How many of the snapshot's claims are evidence-backed (support_status='supported').
 *  Pure projection of the §18.1 claim_support record — shared by the provenance footer,
 *  the publish surfaces, and the hero metrics. */
export function claimSupportSummary(snapshot: ApprovedSnapshotView): {
  readonly supported: number;
  readonly total: number;
} {
  const total = snapshot.claim_support.length;
  const supported = snapshot.claim_support.filter(
    (c) => c.support_status === 'supported',
  ).length;
  return { supported, total };
}

/** The trust badge (operator feedback 2026-06-09): a provenance footer stamped onto every
 *  rendered deliverable, making the claim-provenance work visible IN the document itself —
 *  "N/N claims evidence-linked · content hash · run id". Uses only bold + code spans (the
 *  subset markdownToHtml renders), and only data already in the export shapes (no reviewer
 *  identity — data minimization, spec 019). */
export function provenanceFooterMarkdown(snapshot: ApprovedSnapshotView): string {
  const { supported, total } = claimSupportSummary(snapshot);
  const claims =
    total === 0
      ? 'no factual claims extracted'
      : `${supported}/${total} claims evidence-linked (${Math.round((supported / total) * 100)}%)`;
  const hashPrefix = snapshot.content_hash.slice(0, 12);
  return (
    `**Provenance:** ${claims} · content hash \`${hashPrefix}\` · ` +
    `release run \`${snapshot.release_run_id}\` — generated by shipsignal, ` +
    'human-approved at Gate #2.'
  );
}
 ...
 *  untouched; the JSON export still carries the approved body verbatim). The provenance
 *  trust badge is appended to the RENDERED document only, for the same reason. */
export function renderMarkdownExport(snapshot: ApprovedSnapshotView): string {
  ...
  const footer = provenanceFooterMarkdown(snapshot);
  const title = snapshot.final_title;
  const withTitle =
    title === null || body.trimStart().startsWith('# ') ? body : `# ${title}\n\n${body}`;
  return `${withTitle}\n\n${footer}`;
}
