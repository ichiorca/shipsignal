...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  // Record the manifest-level decision (immutable audit trail), then resume the skill graph thread.
  ...
  try {
    ...
    // The decision is recorded but the resume dispatch didn't start; report 502 so the operator
    // can retry. Log without leaking secrets (the helper already redacts).
